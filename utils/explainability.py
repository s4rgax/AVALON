...
def get_pixel_attention_map(attention_maps, original_image_shape, size: int):
    # Inizializza l'immagine di attenzione combinata
    combined_attention_image = np.zeros(original_image_shape)
    # Inizializza una matrice di conteggio per la normalizzazione
    count_matrix = np.zeros(original_image_shape)
    # Assumendo che l'attenzione sia centrata sul pixel di interesse
    offset = math.floor(size/2)
    for i in range(original_image_shape[0]):
        for j in range(original_image_shape[1]):
            # Calcola gli indici dove inserire la mappa di attenzione 5x5
            ...
            # Estrai la mappa di attenzione corretta
            attention_patch = attention_maps[i * original_image_shape[1] + j]
            # Aggiungi la mappa di attenzione all'immagine combinata
    ...
    # Normalizza l'immagine combinata
    combined_attention_image /= count_matrix
    # Ora 'combined_attention_image' contiene l'immagine di attenzione combinata normalizzata
...
def computeAttentionScene(attention_tensor, ground_truth_matrix, k):
    ...
    X = np.zeros((n + 4*k, m + 4*k))

    for i in range(n):
        for j in range(m):
            if ground_truth_matrix[i, j] == 1:
                X[i+2*k:(i+2*k)+(2*k+1), j+2*k:(j+2*k)+(2*k+1)] = attention_tensor[i, j, :, :]
            else:
                X[i+2*k:(i+2*k)+(2*k+1), j+2*k:(j+2*k)+(2*k+1)] = -1 * attention_tensor[i, j, :, :]

    att_scene = X[2*k:n+2*k, 2*k:m+2*k]
    att_scene /= (n*m)
...
def newComputeAttentionScene(attention_tensor, ground_truth_matrix, k):
    n, m = ground_truth_matrix.shape
